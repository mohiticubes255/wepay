...
class Templates_model extends CI_Model {
    ...
    return $data;
	 ...
	 public function get_setting_data($template_id) {

        $data = array();

        $stmt = "SELECT a.*"

                . " FROM admin_setting AS a"

                . " WHERE a.`id` = " . (int) $template_id;
	 ...
	 public function get_seosetting_data() {

        $data = array();

        $stmt = "SELECT * FROM admin_seo WHERE `status` =0 " ;
...

    {
        ...
        $query = $this->db->query("SELECT * FROM  `email_templates` WHERE view_status='1' ");
